...
-- Operator : constructs a list out of a head and a tail -> x:xs ==> [x,...] where xs is [...]
-- therefore, the deepest call must insert the sum 1+2+3+...+i into an empty list
-- likewise, each call must insert the sum thus far
-- thus, every call must also accumulate the sum that is used as x in x:xs

sums :: Int -> [Int]
sums i = sums' 1 i 0

-- Helper, i is running index, n is upper limit, sum is sum this far and is is list of i's
sums' :: Int -> Int -> Int -> [Int]
sums' i n 0 = sums' (i+1) n 1
sums' i n sum
    |i == n + 1 = sum : []
    |otherwise = sum : (sums' (i+1) n (sum+i))
------------------------------------------------------------------------------
...
sorted []        = True
sorted [x]       = True
sorted [x, xx]   = if x <= xx then True else False
sorted (x:xx:xs) = if x <= xx then sorted (xx:xs) else False
