...
allValues condition (Node x left right) = condition x && allValues condition left && allValues condition right
...
cull _ Empty = Empty
cull val (Node x left right) = if val == x then Empty else Node x (cull val left) (cull val right)
...
isOrdered (Node x left right) = allValues (< x) left && allValues (>x) right && isOrdered left && isOrdered right
...
walk (StepL:steps) (Node x left _ ) = walk steps left
walk (StepR:steps) (Node x _ right) = walk steps right
