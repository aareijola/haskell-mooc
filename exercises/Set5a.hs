...
up pos = MkPosition (getX pos) (getY pos + 1)

-- right increases the x value of a position by one
right :: Position -> Position
right pos = MkPosition (getX pos + 1) (getY pos)
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix a b) = zipWith ( \x y -> (*) 0.5 $ x + y ) (rgb a) (rgb b)
rgb (Invert c) = map ( \x -> 1 - x ) (rgb c)
------------------------------------------------------------------------------
...
toList Empty = []
toList (Pair k v vals) = (k, v) : toList vals
